...
export default function Canvas(props) {
  ...
  let toggleGrayscale = () => {
    ...
    if (scene.children[2]) {
      if (!grayscale) {
        scene.children[2].material = new THREE.MeshPhongMaterial({color: 'gray', flatShading: true});
      } else {
        scene.children[2].material = new THREE.MeshPhongMaterial({vertexColors: THREE.VertexColors, color: 'white', flatShading: true, shininess: 1});
      }
    }
    renderScene();
  }
